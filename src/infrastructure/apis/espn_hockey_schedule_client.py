"""ESPN NHL schedule client — GameEvent parse + B2B detection + 6h in-memory cache.
...
_SCHEDULE_URL_TEMPLATE = (
    "https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams/{team_id}/schedule"
)
...
class EspnHockeyScheduleClient:
    """ESPN NHL team schedule — fetch + 6h cache + back-to-back detection."""
    ...
    def get_team_schedule(
        self, team_id: str, season: int | None = None
    ) -> list[GameEvent]:
        """Takımın schedule'ını çek. 6h cache. HTTP hata → []."""
        if season is None:
            season = self._resolve_season(datetime.now(tz=timezone.utc))
    ...
    def is_back_to_back(self, team_id: str, game_date: datetime) -> bool:
        """Team played yesterday (vs game_date)."""
        season = self._resolve_season(game_date)
        return self.days_since_last_game(team_id, game_date, season) == 1

    # ── Private helpers ──

    @staticmethod
    def _resolve_season(game_date: datetime) -> int:
        """NHL season year: Oct+ → next calendar year (e.g. Oct 2025 → season 2026)."""
    ...
    def _fetch_raw(self, team_id: str, season: int) -> dict | None:
        ...
        try:
            ...
            logger.warning(
                "ESPN NHL schedule fetch failed team=%s season=%s: %s",
                team_id, season, exc,
            )
            return None
    ...
    def _parse_event(self, event: dict) -> GameEvent | None:
        try:
            ...
            status: str = competition["status"]["type"]["description"].lower()
            return GameEvent(
                game_id=game_id,
                date=date,
            ...
            logger.warning(
                "ESPN NHL schedule event parse failed (%r): %s",
                event.get("id"), exc,
            )
            return None
