...
(function (global) {
  ...
  const FEED = {
    ...
    _marketTypeBadge(question, slug, sportsMarketType) {
      const t = FMT.marketType(question, slug, sportsMarketType);
      ...
      return `${this._cardOpen(p.slug, p.force_close_alert, resolving)}
        <div class="feed-top">
          ...
          <div class="feed-badges">
            ...
            ${this._marketTypeBadge(p.question, p.slug, p.sports_market_type)}
          </div>
        </div>
    ...
    _exitedCard(t) {
      ...
      const pnl = Number(t.exit_pnl_usdc || 0);
      // SPEC-Z27: void/iade gerçek kâr/zarar (0.50/hisse) → normal yeşil/kırmızı.
      // "İade" etiketi sebebi açıklar; renk gerçeği yansıtır.
      const pnlCls = FMT.unrealizedClass(pnl);
      ...
      return `${this._cardOpen(t.slug)}
        <div class="feed-top">
          ...
          <div class="feed-badges">
            ...
            ${this._marketTypeBadge(t.question, t.slug, t.sports_market_type)}
          </div>
        </div>
        ${subRowHtml}
        <div class="feed-details">
