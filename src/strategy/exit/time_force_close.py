...
Karar mantığı (hybrid):
  1. ESPN says event ended (period bitti / match final) -> signal
...
def check(
    ...
    # Explicit `is None` — `timeouts.get(...) or` kabul edilemez çünkü 0 falsy →
    # "default'a düş" yanlış davranış (intent: "anında force-close").
...
def _espn_indicates_event_done(status: MatchStatus, market_type: str) -> bool:
    ...
    Period-based markets (e.g. nba_quarter_1_winner): cari period >=2 yeterli.
    Match-level markets (e.g. nba_match_winner): completed=True gerek.
    """
