...
%55'den az ise trade atlanır (`min_favorite_probability`, SPEC-017 ile 0.52 → 0.55).
Underdog value bet'leri alınmıyor — varyans düşürme amacıyla.

### 2.9 Directional Entry (SPEC-017)

**Demir kural:** Bot, bookmaker'ın güçlü favori gördüğü takıma (min %55 güven) makul fiyat aralığında (60-85¢) doğrudan giriş yapar. Edge hesabı yok. Stake, kazanma olasılığı ile orantılı (SPEC-016). Tüm exit kuralları, risk cap'leri, event guard ve manipulation/liquidity filter'ları korunur.
...
### F3. Entry Decision
`strategy/entry/gate.py` giriş kararını orchestrate eder. Tek strateji: **directional entry** (SPEC-017) — bookmaker anchor'dan yön belirle, win_prob ≥ %55 (min_favorite_probability) + 60¢–85¢ fiyat aralığı şartını sağlayan favoriye giriş yap. Edge hesabı yok. Tüm guards (event, liquidity, manipulation, confidence, exposure cap) korunur. (bkz. TDD §6.3)
...
- Price range: 60¢–85¢ (directional entry ile aynı aralık, edge hesabı yok)
