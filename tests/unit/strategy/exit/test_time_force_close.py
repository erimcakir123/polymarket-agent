"""Force-close strategy tests — saf karar mantığı (ana bot — multi-sport).
...
def test_espn_final_returns_signal() -> None:
    ...
    espn = MatchStatus(state="post", period=4, is_completed=True)
    now = datetime.now(timezone.utc)
    result = check(
        pos, espn, now, timeouts={"nba_match_winner": 200},
        market_type="nba_match_winner",
    )
    assert isinstance(result, ForceCloseSignal)
    assert result.reason == "espn_event_ended"


def test_espn_period_2_for_quarter_1_market_returns_signal() -> None:
    pos = _make_position(40)
    espn = MatchStatus(state="in", period=2, is_completed=False)
    now = datetime.now(timezone.utc)
    result = check(
        pos, espn, now, timeouts={"nba_quarter_1_winner": 35},
        market_type="nba_quarter_1_winner",
    )
    assert result is not None
    assert result.reason == "espn_event_ended"


def test_espn_still_in_quarter_1_returns_none() -> None:
    pos = _make_position(20)
...
def test_no_espn_elapsed_exceeds_timeout_returns_signal() -> None:
    pos = _make_position(250)
    now = datetime.now(timezone.utc)
    result = check(
        pos, espn_status=None, now=now,
        timeouts={"nba_match_winner": 180},
        market_type="nba_match_winner",
    )
    assert result is not None
