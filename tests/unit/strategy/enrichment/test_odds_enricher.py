...
def test_enrich_market_tennis_match_totals_returns_bookmaker_over_probability() -> None:
    """SPEC-Z28: tennis Match O/U → bahisçi totals yoluyla Over P(YES) döner (uçtan-uca).

    enrich_market tam zinciri çalıştırır: sport_key resolve → get_odds("totals") →
    event eşleşme → _weighted_totals_average → BookmakerProbability source=bookmaker.
    """
    event = _event_with_bookmakers(
        "Lorenzo Sonego", "Erim Alkaya",
        [
            _bookie_totals("pinnacle", 1.87, 1.95, 22.5),
            _bookie_totals("bet365", 1.90, 1.90, 22.5),
            _bookie_totals("draftkings", 1.91, 1.91, 22.5),
        ],
    )
    # ATP slug → static map'te YOK → _match_tennis_key → get_sports ile çözülür
    client = _client_returning(
        odds_events=[event],
        sports=[{"key": "tennis_atp_roland_garros", "active": True}],
    )
    m = MarketData(
        condition_id="0xz28",
        question="Sonego vs. Alkaya: Match O/U 22.5",
        slug="atp-sonego-alkaya-2026-06-06-match-total-22pt5",
        yes_token_id="y28", no_token_id="n28",
    ...
    result = enrich_market(m, client)
    assert result.probability is not None, f"probability None — fail_reason: {result.fail_reason}"
    assert result.probability.source == "bookmaker"
    assert 0.0 < result.probability.probability < 1.0
    # Over @ ~1.87-1.91 → vig-normalize sonrası ~0.49-0.51 arası beklenir
    assert 0.45 < result.probability.probability < 0.55
