...
class TestMetadata:
    def test_metadata_loaded(self):
        meta = get_table_metadata(_TABLE)
...
class TestMonotonicity:
    def test_more_time_means_more_comeback_room(self):
        ...
        d = 2
        p_60 = leading_team_win_probability_empirical(3, d, 60, table=_TABLE)
        p_300 = leading_team_win_probability_empirical(3, d, 300, table=_TABLE)
        p_1200 = leading_team_win_probability_empirical(3, d, 1200, table=_TABLE)
    ...
    def test_bigger_lead_means_higher_p_win(self):
        ...
        s = 600
        p1 = leading_team_win_probability_empirical(3, 1, s, table=_TABLE)
        p2 = leading_team_win_probability_empirical(3, 2, s, table=_TABLE)
        p3 = leading_team_win_probability_empirical(3, 3, s, table=_TABLE)
        if None in (p1, p2, p3):
...
class TestTrailingConsistency:
    def test_leading_plus_trailing_equals_one(self):
        for p, d, s in [(3, 1, 600), (3, 2, 1200), (2, 1, 1800)]:
            p_lead = leading_team_win_probability_empirical(p, d, s, table=_TABLE)
            p_trail = trailing_team_win_probability_empirical(p, d, s, table=_TABLE)
...
class TestEdgeCases:
    def test_negative_deficit_raises(self):
        with pytest.raises(ValueError):
            leading_team_win_probability_empirical(3, -1, 600, table=_TABLE)

    def test_huge_deficit_clamped(self):
        """7-gol deficit deficit_cap'e clamp edilir, ayni bucket doner."""
        p_5 = leading_team_win_probability_empirical(3, 5, 600, table=_TABLE)
        p_7 = leading_team_win_probability_empirical(3, 7, 600, table=_TABLE)
