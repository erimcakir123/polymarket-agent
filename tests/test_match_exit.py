...
class TestAConfidenceSLFloor:
    """A-confidence positions should have graduated SL floor at 50% (hold-to-resolve)."""

    def _match_started_ago(self, minutes: int) -> str:
        return (datetime.now(timezone.utc) - timedelta(minutes=minutes)).isoformat()

    def test_a_conf_holds_through_30pct_loss(self):
        """A-conf position at 60% elapsed with -30% PnL should NOT exit (below 50% floor).

        Without A-conf override: 60% elapsed tier = -20% base × 0.85 price_mult = -17%
        max_loss, so -30% would exit. With A-conf override: max_loss raised to 50%,
        so -30% is within tolerance.
        """
        from src.match_exit import check_match_exit
        # NHL 150min, 60% elapsed = 90min. Entry 0.55, current 0.385 = -30% PnL.
        # ever_in_profit=True to bypass never-in-profit guard.
        data = _make_pos_data(
            confidence="A",
            entry_price=0.55, current_price=0.385,
    ...
    def test_a_conf_exits_at_51pct_loss(self):
        """A-conf position should exit if loss exceeds 50% floor."""
        from src.match_exit import check_match_exit
        data = _make_pos_data(
            confidence="A",
            entry_price=0.55, current_price=0.26,  # -52.7% PnL
            match_start_iso=self._match_started_ago(105),
            slug="nhl-nyi-car", number_of_games=0,
            category="",
            ai_probability=0.72,
        )
        result = check_match_exit(data)
        # Catastrophic floor fires first at 50% price drop (0.55 * 0.50 = 0.275)
        # 0.26 < 0.275 → catastrophic_floor
        assert result["exit"] is True
        assert result["layer"] in ("catastrophic_floor", "graduated_sl")
    ...
    def test_a_conf_catastrophic_floor_still_applies(self):
        ...
        data = _make_pos_data(
            confidence="A",
            entry_price=0.70, current_price=0.30,  # < entry*0.50 = 0.35
            match_start_iso=self._match_started_ago(60),
    ...
    def test_b_plus_conf_unaffected(self):
        """B+ confidence should still use normal graduated SL (not 50% floor)."""
        from src.match_exit import check_match_exit
        # 85% elapsed NHL (~127min), -20% PnL. Normal tier at 85% = -15% × price_mult.
        # Entry 0.55 → price_mult = 0.85 → max_loss ≈ 0.1275. -20% < -12.75% → exit.
        data = _make_pos_data(
            confidence="B+",
            entry_price=0.55, current_price=0.44,  # -20% PnL
