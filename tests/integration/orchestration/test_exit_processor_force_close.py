"""Force-close integration testleri (SPEC-force-close 2026-05-27, Task 6).
...
def _make_deps_and_pos(orderbook_bids: list[dict]):
    """Common setup: deps mock + 1 deep-loss position 90min old + ESPN None.

    Pozisyon: tennis_first_set_winner, 90 dk önce başlamış → elapsed > 60dk timeout.
    Entry $0.50 × 100 share = $50, current $0.005 → -%99 pnl (deep-loss gate aşılır).
    Time-based path tetiklenir (ESPN None).
    """
    deps = MagicMock()

    # Real config: empty force_close dict default; tennis_first_set_winner = 60min.
    cfg = AppConfig()
    cfg.risk.force_close_timeouts = {"tennis_first_set_winner": 60, "default": 300}
    deps.state.config = cfg

    # Position: 90 dakika önce başlamış (timeout=60'tan büyük).
    ...
    pos = Position(
        ...
        event_id="espn_evt_1",
        slug="test-tennis-match-force-close",
        sport_tag="tennis",
        question="Player A vs Player B set 1",
        match_start_iso=start,
    )
    # SportsMarketType direkt assign etmek lazım — Pydantic v2 enum kabul eder.
    from src.models.enums import SportsMarketType
    pos.sports_market_type = SportsMarketType.TENNIS_FIRST_SET_WINNER
    ...
    # trade_logger.update_on_exit True döner → synth-from-exit path'i tetiklemez.
    deps.trade_logger.update_on_exit = MagicMock(return_value=True)
...
def _stub_monitor_none(monkeypatch):
    ...
    stub o varsayımı testte garantiler. Monitor unit testleri ayrı dosyada
    (test_time_force_close.py + monitor testler) chain'i doğruluyor.
    """
